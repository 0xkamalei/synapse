...
(() => {
  ...
  function extractFeedTextQZone(item: Element): string {
    ...
    const infoElement =
      item.querySelector('.f-info.qz_info_complete') || item.querySelector('.f-info');
    if (infoElement) {
  ...
  function extractFeedImagesQZone(item: Element): string[] {
    ...
    const imgElements = item.querySelectorAll(
      '.f-info img, .f-media img, .f-list-img img, .img-item img, .photo-item img, .pic-item img',
    );
  ...
  function parseQZoneTimeQZone(timeText: string): string {
    ...
    const cnDateMatch = timeText.match(
    ...
    if (cnDateMatch) {
      ...
      const hour = parseInt(cnDateMatch[4]);
      const minute = parseInt(cnDateMatch[5]);
    ...
    if (dateMatch) {
      const year = parseInt(dateMatch[1]) || now.getFullYear();
  ...
  function extractAuthorInfoQZone(item: Element): AuthorInfo {
    const nameElement = (item.querySelector('.f-nick a') ||
      item.querySelector('.f-nick')) as HTMLAnchorElement;
  ...
  function findAllFeedsQZone(): Element[] {
    ...
    for (const selector of selectors) {
      ...
      if (elements.length > 0) {
        console.log(
          `[Synapse] Found ${elements.length} feeds in frame ${window.location.pathname} using selector: ${selector}`,
        );
   ...
   */
  function isTargetURL(targetQQs: string[]): boolean {
    if (!targetQQs || !Array.isArray(targetQQs) || targetQQs.length === 0) return false;

    // Check if any configured QQ matches the current page
    return targetQQs.some((qq) => isQZonePage(qq));
  }
  ...
  async function getPageInfo(): Promise<PageInfo> {
    ...
    let targetQQs = response?.config?.targetQZoneUser || [];
    if (typeof targetQQs === 'string') targetQQs = [targetQQs];

    const isMatched = isTargetURL(targetQQs);
  ...
  async function tryAutoCollect(): Promise<void> {
    ...
    let targetQQs = config.targetQZoneUser;
    if (typeof targetQQs === 'string') targetQQs = [targetQQs];

    if (!targetQQs || targetQQs.length === 0) {
      console.log('[Synapse] Target QZone user not configured, skipping auto-collect');
      return;
    }

    // 1. isTargetURL check
    if (!isTargetURL(targetQQs)) {
      return;
    }
    ...
    if (interval > 0 && lastCollect > 0) {
      ...
      if (hoursSinceLast < interval) {
        console.log(
          `[Synapse] Skipping auto-collect for QZone: last collect was ${hoursSinceLast.toFixed(2)} hours ago (interval: ${interval}h)`,
        );
        return;
      }
    }
    ...
    console.log(
      `[Synapse] QZone auto-collect started for frame ${window.location.hostname}${window.location.pathname}`,
    );
    ...
    if (!targetQQs.includes(pageQQ)) {
      console.log('[Synapse] QZone QQ mismatch, skipping auto-collect', {
        current: pageQQ,
        targets: targetQQs,
      });
      return;
    ...
    console.log(
      `[Synapse] QZone matched target user ${pageQQ}, auto-collecting ${feeds.length} feeds...`,
    );
    ...
    chrome.runtime.sendMessage(
      ...
      (response) => {
        if (response?.success) {
          console.log(
            `[Synapse] QZone auto-collected ${response.collected} feeds from frame ${window.location.pathname}`,
          );
        } else {

 ...
 * 
 * Key DOM elements:
 * - ytd-rich-item-renderer: Video item container (also contains ads and shorts)
 * - yt-lockup-view-model: Contains video metadata
 * - content-id-{videoId}: Class containing the video ID
 * - yt-lockup-metadata-view-model__title: Video title
 * - yt-content-metadata-view-model__metadata-text: Views and timestamp
 */
 ...
 * Supports: https://www.youtube.com/@username/videos
 */
function isTargetURLYoutube(targetChannel?: string): boolean {
    const url = window.location.href;
    
    // Must be a YouTube channel videos page
    const channelVideosPattern = /youtube\.com\/@([^\/]+)\/videos/;
    const match = url.match(channelVideosPattern);
    
    if (!match) {
        return false;
    }
    
    // If no target channel specified, match any channel
    if (!targetChannel) {
        return true;
    }
    
    // Match specific channel (case-insensitive, without @)
    const currentChannel = match[1].toLowerCase();
    const target = targetChannel.replace(/^@/, '').toLowerCase();
    
    return currentChannel === target;
}
 ...
 * The video ID can be in:
 * 1. A class like "content-id-DNF0FJ9l7wo" (older structure)
 * 2. The href of the thumbnail link "/watch?v=DNF0FJ9l7wo" (newer structure)
 */
...
function extractChannelNameYoutube(element: Element): string {
    ...
    const channelLink = element.querySelector('.yt-content-metadata-view-model__metadata-row a[href^="/@"]');
...
function extractViewsYoutube(element: Element): string | null {
    const metadataTexts = element.querySelectorAll('.yt-content-metadata-view-model__metadata-text');
...
function parseRelativeTimeYoutube(relativeTime: string): Date {
    ...
    const patterns: Array<{ regex: RegExp; unit: 'seconds' | 'minutes' | 'hours' | 'days' | 'weeks' | 'months' | 'years' }> = [
 ...
 */
async function tryAutoCollectYoutube(): Promise<void> {
    const config = await chrome.storage.sync.get(['targetYoutubeChannel', 'enabledSources']);
    const targetChannel = config.targetYoutubeChannel as string | undefined;
    const enabledSources = (config.enabledSources as string[]) || [];
    const autoCollect = enabledSources.includes('youtube');

    console.log('[Synapse YouTube] Config:', { targetChannel, enabledSources, autoCollect });

    if (!autoCollect || !targetChannel) {
        console.log('[Synapse YouTube] Auto-collect disabled or no target channel configured');
        return;
    }

    if (!isTargetURLYoutube(targetChannel)) {
        console.log('[Synapse YouTube] Not on target channel page');
        console.log('[Synapse YouTube] Current URL:', window.location.href);
        console.log('[Synapse YouTube] Target channel:', targetChannel);
        return;
    }
    ...
    if (collectedContents.length > 0) {
        console.log(`[Synapse YouTube] Collected ${collectedContents.length} videos, sending to background...`);
        ...
        chrome.runtime.sendMessage({
            ...
            if (response?.success) {
                console.log(`[Synapse YouTube] Successfully processed: ${response.collected} saved, ${response.skipped} skipped`);
            } else {
...
(() => {
    ...
    chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
        if (message.type === 'POP_TO_CONTENT_COLLECT') {
            ...
            getPageInfoYoutube().then(pageInfo => {

...
(() => {
  ...
  function extractWeiboText(postElement: Element): string {
    ...
    const textElement = postElement.querySelector(
      '.wbpro-feed-content .wbpro-feed-ogText, .wbpro-feed-content ._wbtext_1psp9_14',
    );
    let text = '';
  ...
  function extractWeiboImages(postElement: Element): string[] {
    ...
    const imageGallery = postElement.querySelector(
      '[class*="woo-picture"], [class*="feed-images"]',
    );
    if (!imageGallery) {
      return images;
    }
  ...
  async function getPageInfo(): Promise<PageInfo> {
    ...
    let targetUIDs = response?.config?.targetWeiboUser || [];
  ...
  async function tryAutoCollect(): Promise<void> {
    ...
    let targetUIDs = config.targetWeiboUser;
    if (typeof targetUIDs === 'string') targetUIDs = [targetUIDs];
    ...
    if (interval > 0 && lastCollect > 0) {
      ...
      if (hoursSinceLast < interval) {
        console.log(
          `[Synapse] Skipping auto-collect for Weibo: last collect was ${hoursSinceLast.toFixed(2)} hours ago (interval: ${interval}h)`,
        );
        return;
      }
    }

 ...
 */
function extractZsxqVideos(topicElement: Element): string[] {
 ...
 */
async function tryAutoCollectZsxq(): Promise<void> {
    ...
    if (collectedContents.length > 0) {
        console.log(`[Synapse ZSXQ] Collected ${collectedContents.length} topics, sending to background...`);
        ...
        chrome.runtime.sendMessage({
            ...
            if (response?.success) {
                console.log(`[Synapse ZSXQ] Successfully processed: ${response.collected} saved, ${response.skipped} skipped`);
            } else {
